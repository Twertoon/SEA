            ...
            try
            {
                ...
                CustomProperty<SMI.IMyMotorStator, DeltaLimitSwitch<SMI.IMyMotorStator>>.ControlProperty("Virtual Angle",
                    ...
                    (context) => context.Init(
                        ...
                        (block) =>
                        {
                            if (float.IsInfinity(block.LowerLimitDeg) && float.IsInfinity(block.UpperLimitDeg))
                                return MyMath.ShortestAngle(block.Angle, context.Limit);
                            else
                            {
                                if (context.Limit < block.LowerLimitDeg) context.Value = block.LowerLimitDeg;
                                else if (context.Limit > block.UpperLimitDeg) context.Value = block.UpperLimitDeg;
