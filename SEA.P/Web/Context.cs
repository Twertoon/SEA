...
namespace SEA.P.Web
{
    ...
    public class StorageManager : IDisposable
    {
        ...
        private async void CreateAndSeedAsync()
        {
            ...
            await db.CreateTablesAsync<Models.World, Grid, Control, ControlSettings, UserData>();
        }
        ...
        public class TableWorlds
        {
            ...
            public async Task<List<Models.World>> GetAllAsync()
            {
                try
                {
                    var list = await db.Table<Models.World>().ToListAsync();
            ...
            public async Task<Models.World> GetAsync(int id)
            {
                if (id > 0)
                    try
                    {
                        return await db.Table<Models.World>().Where(x => x.Id == id).FirstOrDefaultAsync();
                    }
                    catch { }

                return null;
            }
            public async Task<List<Models.World>> GetWhereAsync(long extId)
            {
                if (extId > 0)
                    try
                    {
                        var worldList = await db.Table<Models.World>().Where(x => x.ExtId == extId).ToListAsync();
            ...
            public async Task<int> AddAsync(Models.World world)
            {
                if (world != null && world.isNew && world.isValid)
                    using (await _lockModify.LockAsync())
                        try
                        {
                            if (await db.Table<Models.World>().Where(x => x.Title == world.Title).CountAsync() > 0)
            ...
            public async Task<List<Models.World>> AddAsync(List<Models.World> worldList)
            {
                if (worldList != null && worldList.Count > 0 && worldList.All(x => x.isNew && x.isValid))
                    using (await _lockModify.LockAsync())
                        try
                        {
                            ...
                            if (await db.Table<Models.World>().Where(x => _titleList.Contains(x.Title)).CountAsync() > 0)
                                return null;
            ...
            public async Task<int> UpdateAsync(Models.World world)
            {
                if (world != null && !world.isNew && world.isValid)
                    using (await _lockModify.LockAsync())
                        try
                        {
                            if (await db.Table<Models.World>().Where(x => x.Id == world.Id).CountAsync() == 0)
                                return (int)ErrorCode.NotExist;//Not exist

                            if (await db.Table<Models.World>().Where(x => x.Title == world.Title && x.Id != world.Id).CountAsync() > 0)
                ...
                if (id > 0)
                    using (await _lockCascadeDelete.LockAsync())
                        try
                        {
                            ...
                            try
                            {
                                _world = await db.GetWithChildrenAsync<Models.World>(id, true);
                            }
                            catch
                            {
                                return (int)ErrorCode.Invalid;
