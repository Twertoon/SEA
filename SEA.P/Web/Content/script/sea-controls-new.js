...
$.widget( "controlunit.sea_button", $.sea_ui.controlunit, {
	...
	_afterCreate: function (){
		...
		this.button = $("<div class='button pointerup'></div>");
		...
		this._on( this.element, {
			"pointerdown .sea-button": "_pointerDown",
			"pointerleave .sea-button": "_pointerLeave",
			"pointerup .sea-button": "_pointerUp"
		});
	},
	
	_do: function ( phase ){
		
		//this.button.attr('class', phase ? 'button pointerdown' : 'button pointerup');
		if(this.options.phase === phase){
			
			Hub.Game.setBlockAction(this.options.eId, this.options.action);
		}
		//console.log("test");		
		//console.log(this.isPrimary, this.pointerId)

	},
	_pointerDown: function ( event ){
		
		this.button.attr('class', 'button pointerdown');
	...
	_pointerLeave: function ( event ){
		...
		this.button.attr('class', 'button pointerleave');
		if(this._allowEvents(event)){
	...
	_pointerUp: function ( event ){
	    ...
	    this.button.attr('class', 'button pointerup');
		if(this._allowEvents(event)){
...
$.widget( "controlunit.sea_slider", $.sea_ui.controlunit, {
	...
	_afterCreate: function (){
		...
		this.slider = $("<div class='slider pointerup' style='" + (vertical ? "height:25" : "width:25") + "px;'></div>");
